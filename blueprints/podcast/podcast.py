...
def index():
    ...
    cursor.execute("SELECT PkIdCad AS id, NomeCad AS name FROM TBCadeira")
    subjects = cursor.fetchall()

    conn.close()

    return render_template('podatituspg1.html', subjects=subjects)

@podcast_bp.route('/select_contents', methods=['POST'])
def select_contents():
    subject_id = request.form.get('subject')
    podcast_name = request.form.get('podcast-name')

    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute("SELECT PkIdArq AS id, TituloArq as title FROM TBArquivo WHERE FkIdCad = ? AND FlgResumoArq = 0", (subject_id,))
    pdfs = cursor.fetchall()

    conn.close()

    return render_template('podatituspg2.html', pdfs=pdfs, podcast_name=podcast_name, subject_id=subject_id)
